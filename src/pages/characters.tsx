...
export default function Characters(prop: {searchCharacter: string}){
    ...
    useEffect(() => {
        axios.get("https://psychonauts-api.herokuapp.com/api/characters")
        .then(({data}) => setCarachters(data))
        .catch((error) => console.log(error));
    }, []);

    function getCharacter(characterName: string, characters: never[] | { name: string; img: string; }[]) {
        if(characterName.length === 0) return characters;
        else{
            let filterCharacterByName = characters.filter(character => character.name.includes(characterName));
